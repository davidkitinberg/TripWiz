...
2. For initial setup and ongoing maintenance, **AdministratorAccess** (or an
   ...
   `[INSERT SCREENSHOT: IAM Console — Administrator IAM User/Role Summary]`
...
2. The guided deployment will prompt you for:
   ...
   * **Parameter values** — the template defines four parameters you must supply:
   ...
   `[INSERT SCREENSHOT: Terminal — sam deploy --guided Parameter Prompts]`
...
4. Once complete, open the **AWS CloudFormation console** and locate your stack to
   ...
   `[INSERT SCREENSHOT: CloudFormation Console — Stack Outputs Tab]`
...
3. Monitor the **CloudFormation console** to watch the stack transition through
   ...
   `[INSERT SCREENSHOT: CloudFormation Console — Stack Events During Update]`
...
`[INSERT SCREENSHOT: Lambda Console — Functions List Filtered to "TripWiz"]`
...
2. On the **Configuration** tab:
   ...
   * **Environment variables** — shows the runtime configuration injected at deploy
   ...
   `[INSERT SCREENSHOT: Lambda Console — TripWiz-Trips Configuration & Environment Variables]`
...
3. Click **Stages → prod** to view the live deployment stage and its **Invoke URL**
   ...
   `[INSERT SCREENSHOT: API Gateway Console — TripWizApi Routes and prod Stage]`
...
2. Review its **Routes**: `$connect`, `$disconnect`, `join`, `leave`, `edit`, `cursor`,
   ...
   `[INSERT SCREENSHOT: API Gateway Console — WebSocket API Routes]`
...
2. Note its configuration:
   ...
   * **TTL attribute:** `ttl` — DynamoDB automatically expires and removes items
   ...
   `[INSERT SCREENSHOT: DynamoDB Console — TripWizTable Overview, Indexes, and TTL Settings]`
...
2. Review its configuration:
   ...
   * **CORS configuration:** Allows `PUT`, `GET`, and `HEAD` from any origin (`*`),
   ...
   `[INSERT SCREENSHOT: S3 Console — TripWizDocumentsBucket Permissions and CORS Configuration]`
...
2. Under **Sign-in experience**, confirm `email` is configured as an
   **auto-verified attribute** — new users must verify their email via a confirmation
   ...
   `[INSERT SCREENSHOT: Cognito Console — TripWizUserPool Sign-in Experience Settings]`
...
1. Under **User pool properties → Lambda triggers**, confirm **Post confirmation** is
   ...
   `[INSERT SCREENSHOT: Cognito Console — TripWizUserPool Lambda Trigger Configuration]`
...
1. In the Cognito console, open **`TripWizUserPool` → Groups** and locate the
   **`Admins`** group (precedence: 1).
   ...
   `[INSERT SCREENSHOT: Cognito Console — Admins Group Membership List]`
...
1. From the Cognito console, open **Users**, search for the affected user by email or
   ...
   `[INSERT SCREENSHOT: Cognito Console — User Profile Detail View]`
...
1. Open the **Amazon Bedrock console** and navigate to **Model access**.
2. Confirm that access to **Anthropic Claude Haiku 4.5**
   (`anthropic.claude-haiku-4-5-*`) is **enabled** for your account/region — this
   must be explicitly requested and approved before `TripWiz-Trips` can successfully
   invoke the model. If optimize requests are failing, this is the first thing to check.

   `[INSERT SCREENSHOT: Bedrock Console — Model Access Status for Claude Haiku 4.5]`
...
2. Under **Place indexes**, locate **`TripWizPlaceIndex`** — confirm its data source
   ...
   `[INSERT SCREENSHOT: Location Service Console — TripWizPlaceIndex Configuration]`
...
2. Confirm that **both** the `SourceEmail` and `AdminEmail` addresses you supplied
   ...
   `[INSERT SCREENSHOT: SES Console — Verified Identities List]`
...
2. Confirm that **`TripWiz-SnsToSes`** is subscribed as a Lambda endpoint — this is
   ...
   `[INSERT SCREENSHOT: SNS Console — TripWiz-Alerts Topic Subscriptions]`
...
1. Open the **Amazon EventBridge console → Scheduler** (or **Rules**, depending on
   ...
   `[INSERT SCREENSHOT: EventBridge Console — Scheduled Rules for Validate and Reminder Lambdas]`
...
2. Click **Retrieve secret value → Edit**.
   ...
   `[INSERT SCREENSHOT: Secrets Manager Console — Editing a Secret's Value]`
...
1. Open the **CloudWatch console → Log groups**. Each Lambda function automatically
   ...
   `[INSERT SCREENSHOT: CloudWatch Console — Log Groups List for TripWiz Lambda Functions]`
...
2. Click into a log group and select a recent **log stream** to view individual
   ...
   `[INSERT SCREENSHOT: CloudWatch Console — Log Stream Detail for TripWiz-WebSocketHandler]`
...
3. Use **CloudWatch Logs Insights** to run structured queries across a function's logs
   ...
   `[INSERT SCREENSHOT: CloudWatch Logs Insights — Query Results for Version-Conflict Errors]`
...
3. Configure each alarm to publish to an **SNS topic** that emails or pages the
   ...
   `[INSERT SCREENSHOT: CloudWatch Console — Alarm Configuration for Lambda Error Rate]`
...
1. Open the **AWS Cost Explorer** console and filter by **service** to see TripWiz's
   ...
   `[INSERT SCREENSHOT: AWS Cost Explorer — Monthly Cost Breakdown by Service]`

2. Compare actuals against the **detailed projected estimate in [`06.md`](06.md)**
...
2. Configure a **monthly cost budget** with a threshold informed by the Folder 06
   ...
   `[INSERT SCREENSHOT: AWS Budgets Console — Monthly Cost Budget Configuration]`
...
- [ ] Every `[INSERT SCREENSHOT: ...]` placeholder above has been replaced with an
      actual, current screenshot from the AWS Management Console for the deployed
      TripWiz stack.
- [ ] Screenshots have any sensitive values (account IDs, ARNs containing account
      numbers, secret values, email addresses) appropriately redacted before
      submission, consistent with good security hygiene.
- [ ] All console steps have been verified against the actual deployed stack — if the
...
- [ ] This written manual is treated as the **primary, authoritative deliverable** for
